## Script for getting and cleaning data from the EMOCEMP preliminary analysis
## --------------  Authors: Rafael Sommer & Bruna Klein -----------------------
## ----------------------------------------------------------------------------
## This script gets raw Qualtrics data from V1 and V2 and set it up for analysis
...
#################################################################################################
####### INSIRA ABAIXO O CAMINHO PARA PASTA COM OS ARQUIVOS E O NOME DOS ARQUIVOS DE DADOS #######
#################################################################################################

dir1 <- "~/Dropbox/Qualificação doutorado" #Insira o caminho para a pasta com os arquivos
file_v1 <- "EMOCEMP_V1_jan20.csv" #Insira o nome do arquivo .csv com dados da visita 1
file_v2 <- "EMOCEMP_V2_jan20.csv" #Insira o nome do arquivo .csv com dados da visita 2
file_mog <- "serologic.testing_MOG.csv"
...
source("emocemp_functions.R")

#Import data
data <- file_v1
emocemp_messy <- read.csv(data, 
                          skip = 1) # Load Data
...
# Infecção e vacinação em uma mesma variável 

emocemp$data_vacina <- dmy (emocemp$data_vacina)
emocemp$data_onset <-dmy (emocemp$data_onset)
# calcular diferença entre a ultima vacina e o inicio da doença em meses
emocemp <- emocemp %>%
    mutate(vacina_m = as.integer (
    (data_onset - data_vacina) / 12 ) )
#  diferenças negativos =NA (vacina após o início dos sintomas)
wrong_dtvacina <- emocemp$vacina_m < 0
emocemp$vacina_m[wrong_dtvacina] <- NA
emocemp <- emocemp %>%
  mutate(vacina_recente = ifelse(vacina_m <= 2, "1", "0") )
emocemp <- emocemp %>% mutate (inf_ou_vac_rec = ifelse(vacina_m==1 | infeccao_2meses == "sim", "1", "0" ))
...
# Merge V2 - warning! (2 V2 do 212!)
emocemp_quali.t <- merge(emocemp_merged_complete, emocemp2, by = "id_paciente", all.x = TRUE)

# Excluir casos por perda de seguimento em 2 visitas, NO Leber, leucodistrofia mais provável, 2 surtos no recrutamento
exclude <- c("103", "104","115","106", "913", "212", "418", "421" )
emocemp_quali.exc <-subset (emocemp_quali.t, !(id_paciente %in% exclude))

# Apenas os já testados para mog e aqp4
emocemp_quali <- emocemp_quali.exc[!is.na(emocemp_quali.exc$mog |  emocemp_quali.exc$aqp4.positivo),]
